...
let beforeTransceive: any = null;

function setBeforeTransceive(callback: any) {
...
const withAndroidPrompt = (fn: any) => {
  async function wrapper() {
    try {
      ...
      const resp = await fn.apply(null);
...
class NfcProxy {
  ...
  readNdefOnce = withAndroidPrompt(async () => {
    const cleanUp = () => {
  ...
  writeNdef = withAndroidPrompt(
    async ({ type, value }: { type: any; value: any }) => {
      let result = false;
  ...
  customTransceiveNfcA = withAndroidPrompt(
    async (commands: any, onPostExecute: any) => {
      let result = false;
      const responses = [];
      ...
      try {
        ...
        let cmdIdx = 0;
        console.log("annnnnnn commands commands", commands);
        for (const command of commands) {
          ...
          if (type === "command") {
            console.warn(
              payload
                .map((byte: any) => ("00" + byte.toString(16)).slice(-2))
                .join(" "),
            );
  ...
  eraseNfcA = withAndroidPrompt(async ({ format = false } = {}) => {
    ...
    try {
      ...
      const [size] = await NfcManager.nfcAHandler.transceive(cmdReadCC);
        ...
        for (const { type, payload } of commands) {
          ...
          if (type === "command") {
            console.log(
              ">>> " +
                payload
                  .map((b: any) => ("00" + b.toString(16)).slice(-2))
                  .join(" "),
            );
  ...
  customTransceiveNfcV = withAndroidPrompt(
    async (commands: any, onPostExecute: any) => {
      let result = false;
      const responses = [];
