  ...
  const RenderItem = useMemo(() => {
    return ({ tx, index }: { tx: any; index: any }) => {
      if (coinSelected?.native) {
        ...
        const arr = tx?.token_transfers?.filter(
          (e: any) =>
            e?.token?.address?.toLowerCase() ===
            coinSelected?.contract?.toLowerCase(),
        );
        if (arr) {
          return arr?.map((txchild: any, j: any) => (
            <View style={styles.content} key={index.toString() + j.toString()}>
              ...
              <View style={styles.items}>
                ...
                {txchild?.from.hash.toLowerCase() ===
                  descyptNEMOWallet(accountWeb?.nemo_address) && (
                  <View style={styles.item}>
                    <MyTextApp style={styles.out}>{t("wallet.out")}</MyTextApp>
                    <Image
                      style={styles.checkIn}
                      source={ICONS.checkIn}
                      alt=""
...
export default function ListTransactionAllComponent({
  coinSelected,
  transactions,
  transactionNextPage,
